...
# Load LangSmith configuration from .env BEFORE any LangChain imports so that
# tracing is active for every LLM call made by the pipeline.
...
# Apply LangSmith settings from .env only when they are not already present in
# the process environment. This keeps the external environment authoritative
# while still making .env work out of the box.
...
# pipeline_v2 explicitly opts in to tracing. If .env disables tracing we still
# enable it, because the goal of this module is LangSmith observability.
# Use --disable-langsmith or set LANGCHAIN_TRACING_V2=false externally to turn
# it off.
...
LOG_FILE = LOGS_DIR / "agent_pipeline_v2.log"
logger.add(
    LOG_FILE,
    rotation="10 MB",
    retention="7 days",
...
except ImportError as exc:  # pragma: no cover - langsmith is required for v2
    raise ImportError(
        "langsmith is required for pipeline_v2. "
...
# Decorators that mark the LLM-invoking pipeline steps for LangSmith.
# Each decorator creates a trace span with run_type="llm" so LangSmith groups
# them under the LLM category and surfaces inputs/outputs/tokens.
...
class RoutedRAGPipeline:
    def __init__(
        ...
        *,
        ...
        logger.info(
            "LangSmith tracing enabled: endpoint={}, project={}",
    ...
    def invoke(self, query: str, *, include_prompt: bool = False) -> dict[str, Any]:
        # The LangChain runnable chain automatically creates a parent trace in
        # LangSmith when LANGCHAIN_TRACING_V2 is active. The LLM spans are added
        # by the @traceable wrappers around the individual model steps.
...
def _configure_langsmith(args: argparse.Namespace) -> None:
    """Apply runtime LangSmith overrides requested from the CLI."""
    if args.langsmith_project:
        os.environ["LANGCHAIN_PROJECT"] = args.langsmith_project
    if args.disable_langsmith:
        os.environ["LANGCHAIN_TRACING_V2"] = "false"
        logger.info("LangSmith tracing disabled by --disable-langsmith")
    else:
        os.environ["LANGCHAIN_TRACING_V2"] = "true"
        logger.info(
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Connected agent pipeline CLI with LangSmith tracing and indexing/query/scope commands"
    )
