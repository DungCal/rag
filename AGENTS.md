...
# Run with input/output safety + PII guards (requires Presidio services + guardrails)
python -m pipelines.agent_pipeline.pipeline run --query "..." --enable-input-guard --enable-output-guard
...
- `pipelines/agent_pipeline/` — LangChain prompt-based routing + Pinecone retrieval + reranking + safety/PII guardrails
...
- The Guardrails safety validators (especially `NSFWText`) can be noisy on long LLM-generated responses. The default `NSFWText` threshold is raised to 0.95 in the pipeline CLI; use `--safety-nsfw-threshold` to tune.
